...
def get_county_attributes(svg_file_name):
    ...
    lst = [(p.getAttribute("id"), p.getAttribute("d")) for p in doc.getElementsByTagName("path")]
    return lst
...
def get_boundary_coordinates(boundary_data):
    ...
    Ignores 'M', 'L, 'z'
    """
    lst = boundary_data.replace("M ","").replace("L ","").replace("z ","").split()
    lst = [tuple(map(float,coordinate.split(","))) for coordinate in lst]
    return lst
