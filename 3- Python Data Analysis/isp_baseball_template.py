...
def filter_by_year(statistics, year, yearid):
    ...
    """
    return [i for i in statistics if int(i[yearid]) == year]
...
def top_player_ids(info, statistics, formula, numplayers):
    ...
    """
    players_staistics = [(player[info['playerid']], formula(info, player)) for player in statistics]
    players_staistics.sort(key=lambda i: i[1], reverse=True)
    return players_staistics[:numplayers]
...
def lookup_player_names(info, top_ids_and_stats):
    ...
    for player in top_ids_and_stats:
        res = (player[1], data[player[0]][info['firstname']], data[player[0]][info['lastname']])
        lookup_res.append("{0:1.3f} --- {1} {2}".format(res[0], res[1], res[2]))
...
def compute_top_stats_year(info, formula, numplayers, year):
    ...
    top_players = top_player_ids(info, statistics, formula, numplayers)
...
def aggregate_by_player_id(statistics, playerid, fields):
    ...
    dictionary = dict()
    fields_dict = {field: 0 for field in fields}
    for stat in statistics:
        dictionary[stat[playerid]] = dict(fields_dict)
        dictionary[stat[playerid]][playerid] = stat[playerid]

    for stat in statistics:
        for field in fields:
            dictionary[stat[playerid]][field] += int(stat[field])
...
def compute_top_stats_career(info, formula, numplayers):
    ...
    """
    statistics = read_csv_as_list_dict(info['battingfile'], info['separator'], info['quote'])
    statistics = aggregate_by_player_id(statistics, info['playerid'], info['battingfields'])
    formated_statistics = [statistics[stat] for stat in statistics]
    top_players = top_player_ids(info, formated_statistics, formula, numplayers)
